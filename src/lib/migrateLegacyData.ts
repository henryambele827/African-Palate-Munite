...
export async function migrateLegacyDataToBrand(brandId: string) {
  ...
  for (const mealDoc of legacyMeals.docs) {
    await setDoc(
      doc(db, "organizations", brandId, "meals", mealDoc.id),
      mealDoc.data(),
    );
    results.meals += 1;
  }
  ...
  for (const orderDoc of legacyOrders.docs) {
    await setDoc(
      doc(db, "organizations", brandId, "orders", orderDoc.id),
      orderDoc.data(),
    );
    results.orders += 1;
  }
  ...
  console.log(
    `Migration complete: ${results.meals} meals, ${results.orders} orders copied to brand ${brandId}.`,
  );
  return results;
}
